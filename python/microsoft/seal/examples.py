...
import pickle
import threading
from microsoft.seal import *

# Helper function: Prints the name of the example in a fancy banner.
def print_example_banner(title):
    if title:
        title_length = len(title)
        banner_length = title_length + 2 + 2 * 10
        banner_top = '*' * banner_length
        banner_middle = '*' * 10 + " " + title + " " + '*' * 10
...
def print_parameters(context):
    ...
    scheme = context_data.parms.scheme
    ...
    print("| poly_modulus_degree:", context_data.parms.poly_modulus_degree)

    # Print the size of the true (product) coefficient modulus.
    print("| coeff_modulus size:",
        context_data.total_coeff_modulus_bit_count, "bits")

    # For the BFV scheme print the plain_modulus parameter.
    if context_data.parms.scheme == scheme_type.BFV:
        print("| plain_modulus:", context_data.parms.plain_modulus.value)

    print("\\ noise_standard_deviation:", context_data.
...
def print_vector(vec, print_size = 4, prec = 3):
    ...
    to_prec = lambda xs: [("{:."+str(prec)+"f}").format(x) for x in xs]

    print()
    if slot_count <= 2 * print_size:
        print("    [ ", end='')
        print(", ".join(to_prec(vec)), end='')
        print(" ]\n")
    else:
        print("    [ ", end = '')
        print(", ".join(to_prec(vec[:print_size])), end='')
        print(", ..., ", end = '')
        print(", ".join(to_prec(vec[-print_size:])), end='')
        print(" ]\n")
...
def main():
    while True:
        print("\nSEAL Examples:\n")
        print(" 1. BFV Basics I")
        print(" 2. BFV Basics II")
        print(" 3. BFV Basics III")
        print(" 4. BFV Basics IV")
        print(" 5. BFV Performance Test")
        print(" 6. CKKS Basics I")
        print(" 7. CKKS Basics II")
        print(" 8. CKKS Basics III")
        print(" 9. CKKS Performance Test")
        print(" 0. Exit")
        ...
        print("\nTotal memory allocated from the current memory pool:",
            (get_pool().alloc_byte_count >> 20), "MB\n")

        try:
            selection = int(input("Run example: "))
        except:
...
def example_bfv_basics_i():
    ...
    parms.coeff_modulus = coeff_modulus_128(2048)
    ...
    print("Encoded", value1, "as polynomial", plain1, "(plain1)")

    value2 = -7
    plain2 = encoder.encode(value2)
    print("Encoded", value2, "as polynomial", plain2, "(plain2)")

    # Encrypting the encoded values is easy.
    encrypted1 = Ciphertext()
    encrypted2 = Ciphertext()
    print("Encrypting plain1: ", end='')
    encryptor.encrypt(plain1, encrypted1)
    print("Done (encrypted1)")

    print("Encrypting plain2: ", end='')
    ...
    print("Decrypting result: ", end='')
    decryptor.decrypt(encrypted1, plain_result)
    print("Done")

    # Print the result plaintext polynomial.
    print("Plaintext polynomial:", plain_result)

    # Decode to obtain an integer result.
    print("Decoded integer:", encoder.decode_int32(plain_result))

def example_bfv_basics_ii():
    ...
    parms.coeff_modulus = coeff_modulus_128(8192)
    ...
    print("Size of a fresh encryption:", encrypted.size)
    ...
    print("Size after squaring:", encrypted.size)
    print("Noise budget after squaring:",
        decryptor.invariant_noise_budget(encrypted), "bits")

    evaluator.square_inplace(encrypted)
    print("Size after second squaring:", encrypted.size)
    ...
    relin_keys16 = keygen.relin_keys(16)
    ...
    print("Size after relinearization:", encrypted.size)
    print("Noise budget after relinearizing (dbc =",
        f"{relin_keys16.decomposition_bit_count}):",
        decryptor.invariant_noise_budget(encrypted), "bits")
    ...
    relin_keys60 = keygen.relin_keys(dbc_max())
    ...
    print("Size after relinearization:", encrypted.size)
    print("Noise budget after relinearizing (dbc =",
        f"{relin_keys60.decomposition_bit_count}):",
        decryptor.invariant_noise_budget(encrypted), "bits")
    ...
    evaluator.square_inplace(encrypted)
    print("Size after third squaring:", encrypted.size)
...
def example_bfv_basics_iii():
    ...
    parms.coeff_modulus = coeff_modulus_128(4096)
    ...
    qualifiers = context.context_data().qualifiers;
    ...
    def print_matrix(matrix):
        ...
        setw = lambda xs: ["{:>3}".format(x) for x in xs]

        print("    [", end='')
        print(",".join(setw(matrix[:print_size])), end='')
        print(", ...,", end='')
        print(",".join(setw(matrix[row_size - print_size:row_size])), end='')
        print(" ]\n    [", end='')
        print(",".join(setw(matrix[row_size:row_size + print_size])), end='')
        print(", ...,", end='')
        print(",".join(setw(matrix[2 * row_size - print_size:2 * row_size])), end='')
        print(" ]\n")
    ...
    pod_result = batch_encoder.decode_int64(plain_result)
...
def example_bfv_basics_iv():
    ...
    # is changed.
    print("Current parms_id:", parms.parms_id)
    print("Changing plain_modulus ...")
    parms.plain_modulus = (1 << 20) + 1
    print("Current parms_id:", parms.parms_id)
    print()
    ...
    public_key = keygen.public_key
    secret_key = keygen.secret_key
    print("parms_id of public_key:", public_key.parms_id)
    print("parms_id of secret_key:", secret_key.parms_id)
    ...
    print("parms_id of plain:", plain.parms_id, "(not set)")
    ...
    context_data = context.context_data()
    while context_data:
        print("Chain index:", context_data.chain_index)
        print("parms_id:", context_data.parms.parms_id)
        print("coeff_modulus primes: ", end='')
        print([x.value for x in context_data.parms.coeff_modulus])
        print("\\")
        print(" \\-->")
    ...
    decryptor.decrypt(encrypted, plain)
    print("Decryption of eighth power:", plain)
    print()
...
def example_ckks_basics_i():
    ...
    print("parms_id of plain:", plain.parms_id)
    print("parms_id of encrypted:", encrypted.parms_id)
    print()
    ...
    print("Scale in plain:", plain.scale)
    print("Scale in encrypted:", encrypted.scale)
    print()
...
def example_ckks_basics_ii():
    ...
    print("Scale in encrypted before squaring:", encrypted.scale)

    # We did this already in the previous example: square encrypted and observe 
    # the scale growth.
    evaluator.square_inplace(encrypted)
    evaluator.relinearize_inplace(encrypted, relin_keys)
    print("Scale in encrypted after squaring:", encrypted.scale,
        f"({math.log2(encrypted.scale)} bits)")
    print("Current coeff_modulus size:",
        context.context_data(encrypted.parms_id)
            .total_coeff_modulus_bit_count, "bits\n")
    ...
    precise_result = [math.pow(x, 8) for x in inputs]
    print("Precise result:")
    print_vector(precise_result)

def example_ckks_basics_iii():
    ...
    scale = float(parms.coeff_modulus[-1].value)
    plain_x = Plaintext()
    encoder.encode(inputs, scale, plain_x)
    encrypted_x1 = Ciphertext()
    ...
    # 120 bits. 
    encrypted_x1_coeff3 = Ciphertext()
    ...
    print("Modulus chain index for encrypted_x3:",
        context.context_data(encrypted_x3.parms_id).chain_index)
    print("Modulus chain index for encrypted_x1:",
        context.context_data(encrypted_x1.parms_id).chain_index)
    print("Modulus chain index for plain_coeff0:",
        context.context_data(plain_coeff0.parms_id).chain_index)
    print()
    ...
    print("Scale in encrypted_x3:", encrypted_x3.scale)
    print("Scale in encrypted_x1:", encrypted_x1.scale)
    print("Scale in plain_coeff0:", plain_coeff0.scale)
    print()
    ...
    print("Modulus chain index for encrypted_result:",
        context.context_data(encrypted_result.parms_id).chain_index)
    print("Scale in encrypted_result:", encrypted_result.scale,
        f"({math.log2(encrypted_result.scale)} bits)")

    # We decrypt, decode, and print the result.
    plain_result = Plaintext()
    decryptor.decrypt(encrypted_result, plain_result)
    result = encoder.decode(plain_result)
    print("Result of PI*x^3 + 0.4x + 1:")
    ...
    print("Current coeff_modulus size for encrypted_result:",
    ...
    plain_integer_scalar = Plaintext()
    ...
    print("Scale in plain_integer_scalar scale:", plain_integer_scalar.scale)
    print("Scale in encrypted_result:", encrypted_result.scale)

    # We decrypt, decode, and print the result.
    decryptor.decrypt(encrypted_result, plain_result)
    result = encoder.decode(plain_result)
    print("Result of 7 * (PI*x^3 + 0.4x + 1):")
    print_vector(result, 3, 7)
...
def example_bfv_performance():
    ...
    def performance_test(context):
        print_parameters(context)
        parms = context.context_data().parms
        ...
        dbc = dbc_max()
        print(f"Generating relinearization keys (dbc = {dbc}): ", end='')
        time_start = time.time()
        relin_keys = keygen.relin_keys(dbc)
        time_end = time.time()
        time_diff = time_end - time_start
        print(f"Done [{int(time_diff * 1000000)} microseconds]")
        ...
        if not context.context_data().qualifiers.using_batching:
            print("Given encryption parameters do not support batching.")
            return
        print(f"Generating Galois keys (dbc = {dbc}): ", end='')
        time_start = time.time()
        gal_keys = keygen.galois_keys(dbc)
        time_end = time.time()
        time_diff = time_end - time_start
        print(f"Done [{int(time_diff * 1000000)} microseconds]")
        ...
        time_batch_sum = 0.0
        time_unbatch_sum = 0.0
        time_encrypt_sum = 0.0
        time_decrypt_sum = 0.0
        time_add_sum = 0.0
        time_multiply_sum = 0.0
        time_multiply_plain_sum = 0.0
        time_square_sum = 0.0
        time_relinearize_sum = 0.0
        time_rotate_rows_one_step_sum = 0.0
        time_rotate_rows_random_sum = 0.0
        time_rotate_columns_sum = 0.0
        ...
        for i in range(batch_encoder.slot_count):
            pod_vector.append(random.getrandbits(64) % plain_modulus.value)
        ...
        for i in range(count):
            ...
            time_start = time.time()
            pod_vector2 = batch_encoder.decode_uint64(plain)
            time_end = time.time()
            time_unbatch_sum += (time_end - time_start)
            if pod_vector2 != pod_vector:
                print(pod_vector)
                print(pod_vector2)
                raise RuntimeError("Batch/unbatch failed. Something is wrong.")
            ...
            if plain2 != plain:
                raise RuntimeError("Encrypt/decrypt failed. Something is wrong.")
            ...
            random_rotation = random.randint(0, row_size - 1)
            time_start = time.time()
        ...
        print("Average batch:", avg_batch, "microseconds")
        print("Average unbatch:", avg_unbatch, "microseconds")
        ...
        print("Average rotate rows one step:", avg_rotate_rows_one_step, "microseconds")
        print("Average rotate rows random:", avg_rotate_rows_random, "microseconds")
        print("Average rotate columns:", avg_rotate_columns, "microseconds")

    parms = EncryptionParameters(scheme_type.BFV)
    parms.poly_modulus_degree = 4096
    parms.coeff_modulus = coeff_modulus_128(4096)
    parms.plain_modulus = 786433
    ...
    print()
    parms.poly_modulus_degree = 16384
    parms.coeff_modulus = coeff_modulus_128(16384)
    parms.plain_modulus = 786433
    performance_test(SEALContext.Create(parms))

    # # Comment out the following to run the biggest example.
    # print()
    # parms.poly_modulus_degree = 32768
    # parms.coeff_modulus = coeff_modulus_128(32768)
    # parms.plain_modulus = 786433
    # performance_test(SEALContext.Create(parms))

def example_ckks_performance():
    ...
    def performance_test(context):
        ...
        pod_vector = []
        for i in range(ckks_encoder.slot_count):
        ...
        for i in range(count):
            ...
            ckks_encoder.encode(pod_vector, 
                float(parms.coeff_modulus[-1].value), plain)
            time_end = time.time()
            time_encode_sum += (time_end - time_start)

            # [Decoding]
            time_start = time.time()
            pod_vector2 = ckks_encoder.decode(plain)
            time_end = time.time()
            ...
            random_rotation = random.randint(0, ckks_encoder.slot_count - 1)
            time_start = time.time()
        ...
        avg_encode = time_encode_sum / count * 1000000
        avg_decode = time_decode_sum / count * 1000000
        avg_encrypt = time_encrypt_sum / count * 1000000
        avg_decrypt = time_decrypt_sum / count * 1000000
        avg_add = time_add_sum / count * 1000000
        avg_multiply = time_multiply_sum / count * 1000000
        ...
        avg_square = time_square_sum / count * 1000000
        avg_relinearize = time_relinearize_sum / count * 1000000
        avg_rescale = time_rescale_sum / count * 1000000
        avg_rotate_one_step = time_rotate_one_step_sum / count * 1000000
        avg_rotate_random = time_rotate_random_sum / count * 1000000
        avg_conjugate = time_conjugate_sum / count * 1000000
        ...
        print("Average rotate vector one step:", avg_rotate_one_step, "microseconds")
        print("Average rotate vector random:", avg_rotate_random, "microseconds")
